...
public class Neo4jNodeAccessor extends StandardAccessorImpl {
	...
	public void onNew(INKFRequestContext aContext) throws Exception {
		...
		// Create Node
		Node vNode = vInstance.createNode();
		// TODO : do adding a label with argument "aGraph" make sense? It was used as the type of a relationShip coming from the ReferenceNode
		vNode.addLabel(Label.label(aGraph));
		// TODO : do adding a label with argument "aEntity" make sense? It was used as the type of another relationShip coming from the ReferenceNode
		vNode.addLabel(Label.label(aEntity));
	...
	public void onDelete(INKFRequestContext aContext) throws Exception {
		...
		if (vNodeRepresentation != null) {
			try {
				...
				// We delete the relationships of the node
